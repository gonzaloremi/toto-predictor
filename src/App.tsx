...
function AppContent() {
  ...
  const enterAppWithNextMatch = useCallback(() => {
    const next = getNextMatch();
    if (next && isRealTeam(next.team1) && isRealTeam(next.team2)) {
      setSelectedMatch(next);
    }
    window.location.hash = '#app';
  }, []);

  if (!user && !loading && hash !== '#app') {
    return <LandingPage onEnterApp={enterAppWithNextMatch} />;
  }
